...
<% int attendanceNum = (int) session.getAttribute("attendanceNum"); %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<script>
    function pushBtn(i) {
        document.punchform.attendanceNum.value = i;
...
<form action="/ninsyo/main" method="get" name="punchform">
    <input type="hidden" name="attendanceNum">
    <% if (attendanceNum == 1) { %>
        <input type="button" value="出勤" id="begin" onclick="pushBtn(1)" disabled>
        <input type="button" value="退勤" id="end" onclick="pushBtn(2)">
    <% } else { %>
        <input type="button" value="出勤" id="begin" onclick="pushBtn(1)">
        <input type="button" value="退勤" id="end" onclick="pushBtn(2)" disabled>
    <% } %>
</form>
<h3>◆勤務一覧</h3>
<table border=1>
    <tr>
        <th>日付</th><th>出勤時刻</th><th>退勤時刻</th><th>勤務時間</th>
    </tr>
